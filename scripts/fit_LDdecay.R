#A modified version of fit_LDdecay.R script of ngsLD software increasing fit level to 10,
#renaming ld files name and decreasing fit_bin_size to 50
#This is a R script fitting LD decay line and draw a plot to show LD decay rate in different ecotype
...
option_list <- list(
  ...
  make_option(c('--fit_bin_size'), action='store', type='numeric', default=50, help='Bin data into fixed-sized windows for fitting. [default %default bps]'),
  make_option(c('--fit_level'), action='store', type='numeric', default=10, help='Fitting level 0) no fitting, best of 1) Nelder-Mead, 2) and BFGS, 3) and L-BFGS-B). [%default]'),
  make_option(c('--plot_group'), action='store', type='character', default='File', help='Group variable'),
  make_option(c('--plot_data'), action='store_true', type='logical', default=TRUE, help='Also plot data points?'),
...
ld_data$File <- str_remove(ld_data$File, "/rds/general/user/rz520/ephemeral/project/")
ld_files$File <- str_remove(ld_files$File, "/rds/general/user/rz520/ephemeral/project/")
# 
